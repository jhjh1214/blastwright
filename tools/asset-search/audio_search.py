    ...
    for i in range(0, len(ordered), 40):
        ...
        for d in get(DETAILS.format(",".join(map(str, batch)))).get("data", []):
            ...
            if ad.get("audioType") not in (("SoundEffect", "Unknown", "Music") if allow_music else ("SoundEffect", "Unknown")) or ad.get("musicAlbum") or ad.get("artist") not in ("", None, c["name"]):
                ...
                rows.append((a["id"], a["name"], a.get("duration"), c["name"], c.get("isVerifiedCreator"), ordered.index(a["id"])))

    rows.sort(key=lambda r: r[5])
    print(f"CUE {cue}: {len(rows)} candidates ({len(ordered)} searched; rejected {rejected})")
    for id_, name, dur, creator, ver, rank in rows[:8]:
        print(f"  {id_:>16}  {dur:>4}s  {name[:44]:<44} by {creator}{' (verified)' if ver else ''}")
