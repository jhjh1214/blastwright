...
- Done and unit-tested (272 tests, three charge types): `ChainSim`, `Seam`, `Schema`, `Validate`.
...
- Also new, unseen: server-wide world boss (`Shared/WorldBoss.luau`, `Client/WorldBossView.luau`), Assay Office (Codex tab, `Shared/Assay.luau`), Blastwright Plus gamepass (`Shared/Plus.luau`), TESTING.md E22, Notice Board contracts (`Shared/Contracts.luau`, Daily button, E20), bottom-right StatusDock (event tiles with icons + Server Goal tile), area-name banner that fades (E19), hub trophy hall (`Client/Trophies.luau`), combat-linked daily/weekly objectives (Slays, Critical), docs/PUBLISH_GUIDE.md. TESTING.md section E18.
