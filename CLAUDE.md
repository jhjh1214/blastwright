...
- Done and unit-tested (292 tests, three charge types): `ChainSim`, `Seam`, `Schema`, `Validate`.
...
- Also new, unseen: Specimen Cabinet + reveals (`Shared/Specimens.luau`, `Client/Reveal.luau`), Blastwright Track + Track Premium pass, daily streak, full audit in docs/AUDIT.md, `tools/lint/ui_qa.py`, TESTING.md E25, Loadout tab + trait tree (`Shared/Loadout.luau`, `Config/Traits.luau`, four charge types incl. Shockwave ring), slower pacing, locked-pad labels (`Client/SiteLabels.luau`), calm mode (`Client/Calm.luau`), TESTING.md E23, docs/PAID_ITEMS_GUIDE.md and docs/SHOP_ART_GUIDE.md, server-wide world boss (`Shared/WorldBoss.luau`, `Client/WorldBossView.luau`), Assay Office (Codex tab, `Shared/Assay.luau`), Blastwright Plus gamepass (`Shared/Plus.luau`), TESTING.md E22, Notice Board contracts (`Shared/Contracts.luau`, Daily button, E20), bottom-right StatusDock (event tiles with icons + Server Goal tile), area-name banner that fades (E19), hub trophy hall (`Client/Trophies.luau`), combat-linked daily/weekly objectives (Slays, Critical), docs/PUBLISH_GUIDE.md. TESTING.md section E18.
