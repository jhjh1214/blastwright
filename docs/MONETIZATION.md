...
**Test it live, not in Studio.** A 2026-09-22 Studio test session logged `[Roblox]...InExperienceShop...Failed to send in-experience shop unified event. Failing Check(s): hasValidSizeAndPosition` — an internal Roblox analytics warning from inside their own CorePackages, nothing this codebase can touch or cause. It's consistent with this native overlay not fully initializing inside Studio's Play/Test mode. To actually check whether it shows your items, join the **published** experience through roblox.com or the Roblox app, not Studio's Play button.
