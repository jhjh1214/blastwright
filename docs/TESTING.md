...
## E31. Shop uses real dashboard icons; one merged Buy button (unseen in Studio)
- Open the Shop: every card for a product/pass with an id should, after a moment, swap its drawn art for the real icon you set on that item in the Creator Dashboard (Client/ProductIcons.luau fetches it live and caches it for the session; only Club and the FREE cards keep drawn art, since they have no product id to query). If an icon never appears, check that item actually has an icon set in the dashboard.
- Every card now has ONE full-width button at the bottom with the price folded in as a small line under "BUY" (Kit.Button's own Sub slot), instead of a separate price chip beside a narrower button. Click test: the entire visible button, including the price line, should register the purchase; nothing beside/around it should do anything.

## E32. Shop icon retry, rebuilt Buy button (unseen in Studio)
- Icons: a product/pass whose icon fails to load (no icon set yet on the dashboard, or a transient hiccup) now retries automatically every 8 seconds instead of getting stuck on the drawn fallback for the rest of the session; check the F9 Output for `[Shop] icon for product/pass <id>: ...` lines naming exactly which id failed and why, so it's obvious which dashboard item still needs an icon set. Fetches are also spaced ~0.15s apart instead of all firing in the same instant when the Shop first opens.
- Buy button: rebuilt as a single plain button (background, price line, and the click are the same object, highest ZIndex in the card) instead of going through the general-purpose Kit.Button. Check every card state: enabled (green), disabled/"Not for sale yet"/"Owned"/"Claimed" (grey) — text should be clearly readable with a dark outline in both, and clicking anywhere on the coloured button (including the price line) should be the only thing that buys.
