...
**Crystal Farm.** the Farm button: a passive income building (6 levels) that fills a store over real time, also while offline (capped at 8 hours) so there is a reason to come back. Upgrading collects first at the old rate. Logic in `Shared/Farm.luau`, save record `Farm` (schema v9); a new farm starts producing when the player first loads after the update.

**Daily Puzzle.** Once per UTC day, the Daily button > "Today's puzzle" (stand at any free pad) gives everyone the same grid (`Puzzle.Seed(day)`), 3 Basic charges and exactly one volley. Upgrades, events and relics are ignored so it is fair. The score is compared with "par", the greedy solver's score on that grid (`Puzzle.Par`); medals Bronze/Silver/Gold/Perfect pay shards, with +10% per consecutive day up to 7. Save: `Puzzle` record (schema v8).

**Combat live events.** The event calendar (`Config/Events.luau`) now also drives combat: Lurker Swarm Friday, Crystal Storm (daily 20:00 UTC hour), Explosive Sunday. Effects LurkerMult, DamageMult and BountyMult are applied server-side; adding a rotating event is one table.

**Collection and boss mastery.** the Codex button also lists every Lurker (with kill counts), boss and relic (with family), shown as "???" until found. Each boss has optional mastery objectives (Titan: Flawless = no hearts lost, Swift Blast = 8 volleys or fewer, Big Finish = a 20+ pop finishing chain, Iron Will = win with exactly 1 heart). Each pays shards once and unlocks a TITLE the player can wear; the worn title floats above their head for everyone to see. Rules in `Shared/Mastery.luau`, objectives in `Config/Mastery.luau`, save records `Collection` and `Mastery` (schema v12).

**Expedition rooms and relic builds.** Each card between floors now shows a ROOM (what the next floor is), a ROUTE (how risky) and a RELIC. Rooms: Mining Vein, Lurker Nest (a Lurker is already awake), Elite Lurker (1.75x health, double bounty, guaranteed relic), Treasure Vault (payouts x2, rare crystals 3x, nothing hides), Chaos Rift (a random event: Crystal Frenzy x3 payouts for -2 charges, Prism Storm 10x rare crystals, Blood Rush awake Lurker with x3 bounty), and Boss (forced before floors 4 and 8). Routes: Safe Path (+1 charge), Greed Path (x2 payouts, -1 charge), Rest Stop (full refill). Relics have families (Echo, Explosion, Combo, Risk) and change rules rather than adding percentages: Shockwave Core (+30% damage), Last Stand (x1.75 damage at 1 heart), Second Wind (a kill refunds a charge), Focused Lens (Critical Mass 4 pops sooner), Bounty Hunter (x2 bounties, tougher Lurkers), Iron Rig (+1 heart), Vampiric Sparks (Critical Mass heals), plus the earlier chain relics. All in `Config/Expedition.luau`.

**Expeditions.** Beside "Start blasting" every pad has "Begin Expedition": a roguelite dive of 8 floors (two per unlocked area). Charges carry over; between floors you pick one of three cards (a route: Steady +1 charge, Rich x2 payouts but -1 charge, Rest refill; plus a relic that bends the chain rules, e.g. double Resonite, Echostones x3, shielded Dreadgeodes) or bank the haul with a depth bonus (+6% per floor cleared, x1.25 for finishing). A collapse keeps only 40%. Logic is `Shared/Expedition.luau` (pure, unit-tested); the server owns the run in `Game.luau`.

the Goals button lists permanent milestones (pops, chain records, seams cleared, shards earned, geode caches, Codex, Prismatic finds, areas unlocked) in three or so rising tiers each, 28 in total. Progress is derived from stats the save already keeps, so nothing extra is tracked; only "claimed" is stored. Each pays shards once (about 233,500 in total across all goals). Ready goals are listed first. The moment a goal becomes claimable a "Goal ready" toast appears (never for goals that were already done when you joined), and a dot on the dock plus a pulsing objective say one can be claimed. Adding a goal is one line in `Config/Achievements.luau`.
